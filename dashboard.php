...
$managerData = $conn->query("SELECT position FROM employees WHERE employee_id = $manager_id AND status != 9999")->fetch_assoc();
if ($managerData['position'] === 'Schichtmeister - Stv.') {
    $filtered = [];
    foreach ($unterstellte_mitarbeiter as $emp_id) {
        // MODIFIZIERT: Archivierte Mitarbeiter ausfiltern (status != 9999)
        $empData = $conn->query("SELECT position FROM employees WHERE employee_id = $emp_id AND status != 9999")->fetch_assoc();
...
// Die Funktion filterGueltigeMitarbeiterIDs wurde in dashboard_helpers.php modifiziert,
// um auch archivierte Mitarbeiter (status = 9999) auszufiltern
